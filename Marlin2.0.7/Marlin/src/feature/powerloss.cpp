...
uint8_t PrintJobRecovery::power_home_opt = 0;
uint8_t PrintJobRecovery::outage_flag = 0;
...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
...
#if ENABLED(POWER_LOSS_STORAGE_Z_VALUE)
		xyze_pos_t leveled = current_position;
		planner.apply_leveling(leveled);	// Current position with leveling applied
		info.zraise = leveled.z;
#else	
		info.zraise = zraise;
#endif	
    ...
    info.feedrate = uint16_t(feedrate_mm_s * 60.0f);
				info.e_relative = uiCfg.e_relative;
  ...
  void PrintJobRecovery::_outage() {
    ...
    if (IS_SD_PRINTING()  || IS_SD_PAUSED()) save(true, zraise);
...
void PrintJobRecovery::resume() {
  ...
  // preheat nozzles before G28, preheat temperature 160
  #if HOTENDS
    #if HAS_HEATED_BED
    thermalManager.temp_bed.target = info.target_temperature_bed;
    #endif
    
    if(thermalManager.temp_hotend[0].celsius < EXTRUDE_MINTEMP)
    {
      HOTEND_LOOP() {
       const int16_t et = EXTRUDE_MINTEMP;
  ...
  TERN_(PREVENT_COLD_EXTRUSION, thermalManager.allow_cold_extrude = info.flag.allow_cold_extrusion);
	...
	gcode.process_subcommands_now_P(PSTR(
	  ...
	  "G28R0"
		...
		#if ENABLED(POWER_LOSS_STORAGE_Z_VALUE)
			sprintf_P(cmd, PSTR("G92.9 E0 Z%s"), dtostrf(info.zraise, 1, 3, str_1));
		#else
  ...
  dtostrf(info.current_position.z, 1, 3, str_1);
	...
	#elif ENABLED(POWER_LOSS_STORAGE_Z_VALUE)
		sprintf_P(cmd, PSTR("G1 Z%s F200"), str_1);
  #else
  ...
  gcode.process_subcommands_now(cmd);
		...
		if(info.e_relative) gcode.process_subcommands_now_P(PSTR("M83"));
