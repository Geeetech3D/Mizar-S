...
typedef unsigned char BLTCommand;
  ...
  #define BLTOUCH_STOW          50
#endif

#define BLTOUCH_SW_MODE        30
#define BLTOUCH_SELFTEST       70
#define BLTOUCH_MODE_STORE     90
#define BLTOUCH_5V_MODE        110
#define BLTOUCH_OD_MODE        130
#define BLTOUCH_RESET          150
...
public:
  ...
  FORCE_INLINE static void _selftest()           {}//{ command(BLTOUCH_SELFTEST, BLTOUCH_DELAY); }
	FORCE_INLINE static void _colorLedOn()				 { command(BLTOUCH_SELFTEST, BLTOUCH_DELAY); }
	
  FORCE_INLINE static void _set_SW_mode()        {}//{ command(BLTOUCH_SW_MODE, BLTOUCH_DELAY); }
	FORCE_INLINE static void _ledOff()				  	 { command(BLTOUCH_SW_MODE, BLTOUCH_DELAY); }	
  ...
  FORCE_INLINE static void _set_5V_mode()        {}//{ command(BLTOUCH_5V_MODE, BLTOUCH_SET5V_DELAY); }	 	
	FORCE_INLINE static void _touch_reset()   		 { command(BLTOUCH_5V_MODE, BLTOUCH_SET5V_DELAY); }
	FORCE_INLINE static void _startSensorChk()			{ command(BLTOUCH_5V_MODE, BLTOUCH_SET5V_DELAY); }	
	
  FORCE_INLINE static void _set_OD_mode()        {}//{ command(BLTOUCH_OD_MODE, BLTOUCH_SETOD_DELAY); }
  FORCE_INLINE static void _touch_calibrate()   { command(BLTOUCH_OD_MODE, BLTOUCH_SETOD_DELAY); }
  FORCE_INLINE static void _mode_store()         {}//{ command(BLTOUCH_MODE_STORE, BLTOUCH_MODE_STORE_DELAY); }
  FORCE_INLINE static void _whiteLedOn()				 { command(BLTOUCH_MODE_STORE, BLTOUCH_MODE_STORE_DELAY); }	
	FORCE_INLINE static void _endSensorChk()			 { command(BLTOUCH_MODE_STORE, BLTOUCH_MODE_STORE_DELAY); }	
