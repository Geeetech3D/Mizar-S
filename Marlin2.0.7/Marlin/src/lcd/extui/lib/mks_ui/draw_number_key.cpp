  ...
  switch (value) {
    ...
    case level_pos_y5:
      ...
      case x_offset:
        ...
        sprintf_P(public_buf_m, PSTR("%.2f"), probe.offset.x);
        #endif
        break;
      case y_offset:
        #if HAS_PROBE_XY_OFFSET
        sprintf_P(public_buf_m, PSTR("%.2f"), probe.offset.y);
        #endif
        break;
      case z_offset:
        sprintf_P(public_buf_m, PSTR("%.2f"), probe.offset.z);
        break;
    #endif
    case load_length:
      ...
      case z_offset:
        ...
        if (WITHIN(z, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX))
        {
        	if (queue.length <= (BUFSIZE - 3)) {
	        	tmpx = (float)X_BED_SIZE/2;
						tmpy = (float)Y_BED_SIZE/2;
	        	probe.offset.z = z;
						queue.enqueue_now_P(PSTR("G28"));
						queue.enqueue_now_P(PSTR("G1 Z10"));
	          ZERO(public_buf_l);
	          sprintf_P(public_buf_l, PSTR("G1 X%4.1f Y%4.1f Z0"), tmpx, tmpy);
...
void lv_draw_number_key(void) {
  ...
  lv_obj_t *KeyPoint = NULL, *KeyConfirm = NULL, *KeyReturn = NULL, *KeyBack = NULL;
  lv_obj_t *Minus = NULL;
  ...
  lv_obj_t *labelKeyPoint = NULL, *labelKeyConfirm = NULL, *labelKeyReturn = NULL, *labelKeyBack = NULL;
  ...
  */
  ...
  lv_obj_set_event_cb_mks(buttonValue, NULL, ID_NUM_KEY1, NULL, 0);
  ...
  KeyReturn = lv_btn_create(scr, NULL);                                  /*Add a button the current screen*/
  lv_obj_set_pos(KeyReturn, 320, 140);                                   /*Set its position*/
  lv_obj_set_size(KeyReturn, 68, 40);
  lv_obj_set_event_cb_mks(KeyReturn, event_handler, ID_NUM_RETURN, NULL, 0);
  lv_btn_set_style(KeyReturn, LV_BTN_STYLE_REL, &style_num_key_pre);     /*Set the button's released style*/
  lv_btn_set_style(KeyReturn, LV_BTN_STYLE_PR, &style_num_key_rel);      /*Set the button's pressed style*/
  //lv_btn_set_layout(KeyReturn, LV_LAYOUT_OFF);
  labelKeyReturn = lv_label_create(KeyReturn, NULL);                      /*Add a label to the button*/
  lv_label_set_text(labelKeyReturn, machine_menu.key_return);
  lv_obj_align(labelKeyReturn, KeyReturn, LV_ALIGN_CENTER, 0, 0);
