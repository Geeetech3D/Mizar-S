...
static uint8_t fanSpeed_present_value;
    ...
    case ID_F_ADD:
      if (event == LV_EVENT_CLICKED) {
        // nothing to do
				...
				fanSpeed_present_value++;
				if(fanSpeed_present_value > 100)
					fanSpeed_present_value = 100;
				if(fanSpeed_present_value == 100)
					fanSpeed = 255;
				else if(fanSpeed_present_value == 50)
					fanSpeed = 127;
				else
					fanSpeed = fanSpeed_present_value*255/100;
    ...
    case ID_F_DEC:
      if (event == LV_EVENT_CLICKED) {
        // nothing to do
				...
				if(fanSpeed_present_value != 0) 
					fanSpeed_present_value--;
				if(fanSpeed_present_value == 50)
					fanSpeed = 127;
				else 
					fanSpeed = fanSpeed_present_value*255/100;
...
void lv_draw_fan(void) {
  ...
  lv_obj_clear_protect(buttonAdd, LV_PROTECT_FOLLOW);
    ...
    lv_imgbtn_set_src(buttonHigh, LV_BTN_STATE_PR, "F:/bmp_clear.bin");
    ...
    lv_imgbtn_set_src(buttonMid, LV_BTN_STATE_PR, "F:/bmp_clear.bin");
  ...
  lv_obj_set_style(fanText, &tft_style_label_rel);
	...
	fanSpeed_present_value = ((thermalManager.fan_speed[0]*100+128)/255)%101;
  disp_fan_value();
}
...
void disp_fan_value() {
  ...
  sprintf_P(buf1, PSTR("%3d"), fanSpeed_present_value);
