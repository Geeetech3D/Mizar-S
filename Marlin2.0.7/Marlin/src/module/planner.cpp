	...
	float ftempVal = 1.0;
		...
		raw = d + level_fulcrum;
			...
			if(auto_manu_level_sel){
				ftempVal = fade_scaling_factor ? fade_scaling_factor * bilinear_z_offset(raw) : 0.0;
			}else{
				ftempVal =  mbl.get_z(raw
				#if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
				, fade_scaling_factor
				#endif
				);
			...
			ftempVal = fade_scaling_factor ? fade_scaling_factor * ubl.get_z_correction(raw) : 0.0;
		#endif
