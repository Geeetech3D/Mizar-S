...
bool Endstops::x_manual_limit = 0;
bool Endstops::y_manual_limit = 0;
bool Endstops::z_manual_limit = 0;
...
void Endstops::validate_homing_move()
{
    if(trigger_state()) hit_on_purpose();
        ...
        if(gCfgItems.language == LANG_SIMPLE_CHINESE)
            kill(MSG_KILL_HOMING_FAILED_CN);
        else
#endif
...
{
#if ENABLED(BLTOUCH)
    if(auto_manu_level_sel) bltouch._set_SW_mode();
#endif
...
#if ENABLED(BLTOUCH)
    if(auto_manu_level_sel) bltouch._reset_SW_mode();
#endif
