...
inline void say_default_()
{
    SERIAL_ECHOPGM("#define DEFAULT_");
}
                ...
                if(!heated) {                                             // If not yet reached target...
                    if(current_temp > next_watch_temp) {                    // Over the watch temp?
                        ...
                        if(gCfgItems.language == LANG_SIMPLE_CHINESE)
                            _temp_error(heater_id, str_t_heating_failed, MSG_HEATING_FAILED_LCD_CN);
                        else
#endif
...
#if ENABLED(TFT_LVGL_UI)
                    if(gCfgItems.language == LANG_SIMPLE_CHINESE)
                        _temp_error(heater_id, str_t_thermal_runaway, MSG_THERMAL_RUNAWAY_CN);
                    else
#endif
...
#define _SET_EXTRUDER_PID() do { \
          ...
          updatePID(); }while(0)
            ...
            if(set_result) {
#if HAS_PID_FOR_BOTH
                if(isbed) _SET_BED_PID();
                else _SET_EXTRUDER_PID();
#elif ENABLED(PIDTEMP)
                _SET_EXTRUDER_PID();
#else
                _SET_BED_PID();
...
#if HAS_AUTO_FAN_6
        case 6:
            _UPDATE_AUTO_FAN(E6, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
#endif
#if HAS_AUTO_FAN_7
        case 7:
            _UPDATE_AUTO_FAN(E7, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
#endif
...
#if ENABLED(TFT_LVGL_UI)
    if(gCfgItems.language == LANG_SIMPLE_CHINESE)
        _temp_error(heater_id, PSTR(STR_T_MAXTEMP), PSTR(MSG_ERR_MAXTEMP_CN));
    else
#endif
...
#if ENABLED(TFT_LVGL_UI)
    if(gCfgItems.language == LANG_SIMPLE_CHINESE)
        _temp_error(heater_id, PSTR(STR_T_MINTEMP), PSTR(MSG_ERR_MINTEMP_CN));
    else
#endif
...
void Temperature::manage_heater()
{
...
#if ENABLED(TFT_LVGL_UI)
                if(gCfgItems.language == LANG_SIMPLE_CHINESE)
                    _temp_error((heater_id_t)e, str_t_heating_failed, MSG_HEATING_FAILED_LCD_CN);
                else
#endif
    ...
    if(watch_bed.elapsed(ms)) {         // Time to check the bed?
        if(degBed() < watch_bed.target) { // Failed to increase enough?
            if(thermalManager.temp_bed.target < WATCH_BED_TEMP_STOP_TEMP) {
#if ENABLED(TFT_LVGL_UI)
                if(gCfgItems.language == LANG_SIMPLE_CHINESE)
                    _temp_error(H_BED, str_t_heating_failed, MSG_HEATING_FAILED_LCD_CN);
                else
#endif
                    _temp_error(H_BED, str_t_heating_failed, GET_TEXT(MSG_HEATING_FAILED_LCD));
            } else {
                if(watch_bed.target >= WATCH_BED_TEMP_STOP_TEMP) {
                    if((watch_bed.target - WATCH_BED_TEMP_STOP_TEMP) >= 5)
                        setTargetBed(watch_bed.target - 5);
                    else
                        setTargetBed(WATCH_BED_TEMP_STOP_TEMP);
                }
            }
        } else                                                          // Start again if the target is still far off
            start_watching_bed();
...
#define CHAMBER_CHECK_INTERVAL 1000UL
    ...
    if(watch_chamber.elapsed(ms)) {               // Time to check the chamber?
        if(degChamber() < watch_chamber.target) {   // Failed to increase enough?
            if(gCfgItems.language == LANG_SIMPLE_CHINESE)
                temp_error(H_CHAMBER, str_t_heating_failed, MSG_HEATING_FAILED_LCD_CN);
            else
                temp_error(H_CHAMBER, str_t_heating_failed, GET_TEXT(MSG_HEATING_FAILED_LCD));
        } else
...
#if ENABLED(TFT_LVGL_UI)
                uiCfg.maxtemp_except_flag = 1;
                uiCfg.temp_except_id = e;
#else
...
#if HAS_TEMP_PROBE
    case PrepareTemp_PROBE:
        HAL_START_ADC(TEMP_PROBE_PIN);
        break;
    case MeasureTemp_PROBE:
        ACCUMULATE_ADC(temp_probe);
        break;
#endif
...
#if HAS_TEMP_ADC_6
    case PrepareTemp_6:
        HAL_START_ADC(TEMP_6_PIN);
        break;
    case MeasureTemp_6:
        ACCUMULATE_ADC(temp_hotend[6]);
        break;
#endif

#if HAS_TEMP_ADC_7
    case PrepareTemp_7:
        HAL_START_ADC(TEMP_7_PIN);
        break;
    case MeasureTemp_7:
        ACCUMULATE_ADC(temp_hotend[7]);
        break;
#endif
    ...
    case Measure_ADC_KEY:
        if(!HAL_ADC_READY())
            ...
            if(raw_ADCKey_value <= 900UL * HAL_ADC_RANGE / 1024UL) {
                ...
                if(ADCKey_count > 0) ADCKey_count++;
                else ADCKey_pressed = false;
                if(ADCKey_pressed) {
                    ADCKey_count = 0;
