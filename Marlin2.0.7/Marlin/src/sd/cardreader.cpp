...
extern uint8_t return_printui_flag;
...
void CardReader::manage_media() {
  ...
  if (ui.detected()) {
	...
	if(recovery.power_home_opt==0 && stat==0)
		recovery.power_home_opt = 1;
	//interrupted file unexists
	if(recovery.power_home_opt == 1){
		recovery.power_home_opt = 2;
      ...
      if (file_subcall_ctr > SD_PROCEDURE_DEPTH - 1) {
        SERIAL_ERROR_MSG("Exceeded max SUBROUTINE depth:" STRINGIFY(SD_PROCEDURE_DEPTH));
				...
				if(gCfgItems.language == LANG_SIMPLE_CHINESE)
					kill(MSG_KILL_SUBCALL_OVERFLOW_CN);
				else
        #endif
