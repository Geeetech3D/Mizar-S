...
G29_TYPE ABL_G29()
{
    TERN_(TFT_LVGL_UI, uiCfg.auto_leveling_point_status = leveling_doing);
    SERIAL_ECHO_MSG("Auto Leveling Doing.");
...
// Deploy certain probes before starting probing
...
#else // !PROBE_MANUALLY
    {
        ...
        for(PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END && !isnan(measured_z); PR_OUTER_VAR++) {
            ...
            for(PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
                ...
                if(TERN0(IS_KINEMATIC, !probe.can_reach(probePos)))
                    continue;
#if ENABLED(TFT_LVGL_UI)
                if(uiCfg.auto_leveling_point_num > int(pt_index))
                    continue;
                uiCfg.auto_leveling_point_num = int(pt_index);
#endif

                if(verbose_level)
                    SERIAL_ECHOLNPAIR("Probing mesh point ", int(pt_index), "/", abl_points, ".");
...
#endif
                {
                    ...
                    if(abs(probe.zDiscrepancy) > 0.5) {	// check discrepancy for pressure touch sensor
                        pt_index--;
                        PR_INNER_VAR -= inInc;
                        //SERIAL_ECHOLNPAIR("#Leveling#probe again ", pt_stable_cnt+1);
                        if((++pt_stable_cnt) >= 3) {
                            measured_z = NAN;
                            pt_stable_cnt = 0;
                            //SERIAL_ECHO_MSG("#Leveling#go home and probe again");
                        } else
                            continue;
                    } else
                        pt_stable_cnt = 0;
        ...
        LOOP_L_N(i, 3) {
            if(verbose_level)
                SERIAL_ECHOLNPAIR("Probing point ", int(i + 1), "/3.");
    ...
    if(!isnan(measured_z)) {
        ...
        if(!dryrun) {
            ...
            if(NEAR(current_position.x, probePos.x - probe.offset_xy.x) && NEAR(current_position.y, probePos.y - probe.offset_xy.y)) {
        ...
        TERN_(TFT_LVGL_UI, uiCfg.auto_leveling_point_status = leveling_done); //level over
        SERIAL_ECHO_MSG("Auto Leveling Done.");
    }                                                   // !isnan(measured_z)
...
inline void echo_not_entered(const char c)
{
    SERIAL_CHAR(c);
    SERIAL_ECHOLNPGM(" not entered.");
}
...
#if ENABLED(TFT_LVGL_UI)
        if(uiCfg.auto_leveling_force_stop) {
            uiCfg.auto_leveling_point_status = leveling_stop;
            SERIAL_ECHO_MSG("Auto Leveling Stop.");
