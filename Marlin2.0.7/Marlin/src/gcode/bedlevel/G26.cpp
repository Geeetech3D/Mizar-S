...
mesh_index_pair find_closest_circle_to_print(const xy_pos_t &pos) {
  ...
  GRID_LOOP(i, j) {
    if (!circle_flags.marked(i, j)) {
      ...
      const xy_pos_t m = auto_manu_level_sel?{ _ABL_GET_MESH_X(i), _ABL_GET_MESH_X(j) }:{ _MBL_GET_MESH_X(i), _MBL_GET_MESH_X(j) };
      ...
      if (circle_flags.marked(i, j) && circle_flags.marked(i + 1, j)) {   // Test whether a leftward line can be done
        if (!horizontal_mesh_line_flags.marked(i, j)) {
          ...
          if(auto_manu_level_sel){
          	s.x = _ABL_GET_MESH_X(  i  ) + (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // right edge
         	 	e.x = _ABL_GET_MESH_X(i + 1) - (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // left edge
          }else{
						s.x = _MBL_GET_MESH_X(  i  ) + (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // right edge
         	 	e.x = _MBL_GET_MESH_X(i + 1) - (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // left edge
					}
      ...
      if (j < (GRID_MAX_POINTS_Y)) {  // Can't connect to anything further back than GRID_MAX_POINTS_Y.
        ...
        if (circle_flags.marked(i, j) && circle_flags.marked(i, j + 1)) {   // Test whether a downward line can be done
          if (!vertical_mesh_line_flags.marked(i, j)) {
            ...
            if(auto_manu_level_sel){
	            s.y = _ABL_GET_MESH_Y(  j  ) + (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // top edge
	            e.y = _ABL_GET_MESH_Y(j + 1) - (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // bottom edge
		          s.x = e.x = constrain(_ABL_GET_MESH_X(i), X_MIN_POS + 1, X_MAX_POS - 1);
            }else{
							s.y = _MBL_GET_MESH_Y(  j  ) + (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // top edge
	            e.y = _MBL_GET_MESH_Y(j + 1) - (INTERSECTION_CIRCLE_RADIUS - (CROSSHAIRS_SIZE)); // bottom edge
		          s.x = e.x = constrain(_MBL_GET_MESH_X(i), X_MIN_POS + 1, X_MAX_POS - 1);
						}
